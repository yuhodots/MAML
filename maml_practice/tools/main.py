...
Usage Instructions:
    ...
    To script evaluation, use the '--train=False' flag and the '--test_set=True' flag to use the test set.
...
flags.DEFINE_bool('resume', True, 'resume training if there is a models available')
flags.DEFINE_bool('train', True, 'True to train, False to test.')
flags.DEFINE_integer('test_iter', -1, 'iteration to load models (-1 for latest models)')
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr!=0) and itr % SAVE_INTERVAL == 0:
            saver.save(sess, FLAGS.logdir + '/' + exp_string + '/models' + str(itr))
...
def main():
    ...
    if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'omniglot':
        ...
        if FLAGS.train: # only construct training models if needed
            random.seed(5)
    ...
    if FLAGS.train == False:
        # change to original meta batch size when loading models.
    ...
    if FLAGS.resume or not FLAGS.train:
        ...
        if FLAGS.test_iter > 0:
            model_file = model_file[:model_file.index('models')] + 'models' + str(FLAGS.test_iter)
        if model_file:
            ind1 = model_file.index('models')
            resume_itr = int(model_file[ind1+5:])
            print("Restoring models weights from " + model_file)
